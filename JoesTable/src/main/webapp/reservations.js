function displayReservations() {
    fetch('http://localhost:8080/JoesTable/reservationServlet')
       .then(response => {
...
function displayResults(data) {
    ...
    switch (criterion) {
        case 'recent-asc':
            data.sort((a, b) => a.date.localeCompare(b.name));
            break;
        case 'recent-desc':
            data.sort((a, b) => b.date.localeCompare(a.name));
            break;
    }
    ...
    data.forEach(business => {
	    ...
	    var date = business.date;
	    var time = business.time;
	    var notes = business.notes;
	    ...
	    resultElement.innerHTML = `
	        <div class="business-result">
			    ...
			    <div class="business-info">
			        <h2>${name}</h2>
			        <p class="business-address">Date: ${date}</p>
			        <p class="business-address">Time: ${time}</a>
			    </div>
			</div>
	    `;
